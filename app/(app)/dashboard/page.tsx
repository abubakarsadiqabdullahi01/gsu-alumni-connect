...
function calcProfileCompletion(input: {
  email: string | null;
  phone: string | null;
  ...
  const checks = [
    ...
    Boolean(input.nyscState),
    Boolean(input.nyscYear),
    input.hasEmployment,
    input.hasEducation,
    input.hasSkills,
  ];
...
async function AdminExecutiveDashboard() {
  const now = new Date();
      ...
      <div className="flex-1 space-y-6 p-4 md:p-6">
        <Card className="overflow-hidden border-primary/20">
          ...
          <CardHeader className="flex flex-row items-start justify-between gap-4">
            <div>
              <CardTitle className="text-2xl font-extrabold tracking-tight">Admin Command Center</CardTitle>
              <CardDescription>
                Live system health, network growth, and platform operations from production data.
              </CardDescription>
            </div>
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-sky-700">{monthGrowth} vs previous month</p>
                </div>
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-emerald-700">{activationRate}% activation rate</p>
                </div>
            ...
            <CardContent className="p-5">
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-xs text-violet-700">Real-time engagement indicator</p>
                </div>
        ...
        <div className="grid gap-6 xl:grid-cols-[1.2fr_1fr]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Jobs, events, groups, mentorship, and uploads.</CardDescription>
            </CardHeader>
            <CardContent className="grid gap-3 sm:grid-cols-2">
              <div className="rounded-lg border p-4">
                ...
                <p className="text-xs text-muted-foreground">
                  {jobsTotal.toLocaleString()} total, {applicationsTotal.toLocaleString()} applications
                </p>
              </div>
              <div className="rounded-lg border p-4">
              ...
              <div className="rounded-lg border p-4">
                ...
                <p className="mt-1 text-2xl font-bold">{groupsTotal.toLocaleString()} groups</p>
                <p className="text-xs text-muted-foreground">{groupMembersTotal.toLocaleString()} group memberships</p>
              </div>
              <div className="rounded-lg border p-4">
                ...
                <p className="text-xs text-muted-foreground">{mentorshipPending.toLocaleString()} pending requests</p>
              </div>
              <div className="rounded-lg border p-4 sm:col-span-2">
                <p className="text-xs text-muted-foreground">Profile Completion</p>
                <div className="mt-2 flex items-center justify-between text-sm">
                  <span>{profileCompletedCount.toLocaleString()} of {totalGraduates.toLocaleString()} graduates</span>
          ...
          <Card>
            <CardHeader>
              ...
              <CardDescription>Top faculties by alumni population.</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {topFaculties.length === 0 ? (
                <p className="text-sm text-muted-foreground">No faculty distribution data available.</p>
              ) : (
                topFaculties.map((f) => (
                  <div key={f.name}>
        ...
        <div className="grid gap-6 xl:grid-cols-3">
          <Card>
            <CardHeader>
              <CardTitle className="flex items-center gap-2 text-base"><Upload className="size-4" /> Recent Uploads</CardTitle>
            </CardHeader>
            <CardContent className="space-y-3">
              ...
              {recentEvents.length === 0 ? (
                ...
                recentEvents.map((e) => (
                  <div key={e.id} className="rounded-lg border p-3">
                    ...
                    <div className="mt-1 flex items-center justify-between">
                      <Badge variant={e.isCancelled ? "destructive" : "secondary"}>
        ...
        <Card>
          ...
          <CardContent className="grid gap-2 sm:grid-cols-2 xl:grid-cols-4">
            ...
            <Button asChild variant="outline" size="sm">
              <Link href="/admin/achievements"><GraduationCap className="mr-1.5 size-4" />Moderate Achievements</Link>
            </Button>
            <Button asChild variant="outline" size="sm">
...
async function GraduateDashboard(userId: string) {
  const graduate = await prisma.graduate.findUnique({
    ...
    select: {
      ...
      activityFeed: {
        orderBy: { createdAt: "desc" },
        take: 6,
        select: { id: true, headline: true, actionType: true, createdAt: true },
      },
    },
  });
        ...
        <div className="p-6">
          <Card>
            <CardHeader>
              ...
              <CardDescription>
                Your graduate profile is not linked yet. Contact admin support.
              </CardDescription>
            </CardHeader>
          </Card>
        </div>
  ...
  const completion = calcProfileCompletion({
    ...
    hasEmployment: graduate._count.employment > 0,
    hasEducation: graduate._count.education > 0,
    hasSkills: graduate._count.skills > 0,
  });
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
          <div>
            <h1 className="text-2xl font-extrabold tracking-tight">
              Welcome back, {graduate.fullName}
            </h1>
            <p className="text-sm text-muted-foreground">
              Registration: {graduate.registrationNo}
            </p>
          </div>
          <SignOutButton variant="outline" size="sm" />
        </div>
          ...
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                ...
                <p className="text-2xl font-bold">{graduate.profileViews}</p>
              </div>
          ...
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                ...
                <p className="text-2xl font-bold">{graduate._count.jobApplications}</p>
              </div>
          ...
          <Card>
            <CardContent className="flex items-center justify-between p-5">
              <div>
                ...
                <p className="text-2xl font-bold">{graduate._count.groupMemberships}</p>
              </div>
        ...
        <div className="grid gap-6 lg:grid-cols-[1fr_340px]">
          <Card>
            <CardHeader>
              ...
              <CardDescription>Your latest profile and platform activity</CardDescription>
            </CardHeader>
            <CardContent className="space-y-3">
              {graduate.activityFeed.length === 0 ? (
                <p className="text-sm text-muted-foreground">No activity yet.</p>
              ) : (
                graduate.activityFeed.map((item) => (
          ...
          <div className="space-y-6">
            <Card>
              <CardHeader>
                ...
                <CardDescription>
                  {graduate.profileCompleted ? "Profile completed" : "Complete your profile to improve visibility"}
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-3">
...
export default async function DashboardPage() {
  ...
  return <>{await GraduateDashboard(session.user.id)}</>;
}
